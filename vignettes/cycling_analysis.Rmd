...
At this point I should apologise that `read_*()` functions lack portability. This is a shortfall of the package that results from the complex file structures of most cycling head-units. It is also one I would like to rectify in future releases.
...
A simpler and slightly more efficient version of this function is provided as `mmv2()`. This can be used with any numeric vector (rather than just "cycleRdata" objects, as above), with `windows` given in *row* units. Hence, to achieve the above:
